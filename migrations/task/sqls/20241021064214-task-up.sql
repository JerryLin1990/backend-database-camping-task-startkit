...
FROM "COACH" WHERE user_id = (SELECT id FROM "USER" WHERE email = 'muscle@hexschooltest.io');

INSERT INTO "COACH_LINK_SKILL" (coach_id, skill_id)
SELECT 
    id AS coach_id, 
    (SELECT id FROM "SKILL" WHERE name = '有氧運動') AS skill_id
FROM "COACH" 
...
SELECT 
    id AS coach_id, 
    (SELECT id FROM "SKILL" WHERE name = '復健訓練') AS skill_id
FROM "COACH" 
WHERE user_id = (SELECT id FROM "USER" WHERE email = 'starplatinum@hexschooltest.io');
...
SELECT user_id,SUM(purchased_credits) as total FROM "CREDIT_PURCHASE" WHERE user_id = (SELECT id FROM "USER" WHERE name = '王小明')
Group by user_id;
-- 5-7. 查詢：計算用戶王小明的已使用堂數，顯示須包含以下欄位： user_id , total。 (需使用到 Count 函式與 Group By)
SELECT user_id,Count(course_id) as total FROM "COURSE_BOOKING" WHERE user_id = (SELECT id FROM "USER" WHERE name = '王小明')
AND status = '上課中'
Group by user_id;
...
SELECT u.name AS 教練名稱,c.experience_years AS 經驗年數,s.name 專長名稱 FROM "COACH" AS c
...
SELECT s.name AS 專長名稱,count( cls.coach_id) AS coach_total FROM "COACH" AS c
INNER JOIN "USER" u ON c.user_id = u.id
INNER JOIN "COACH_LINK_SKILL" cls ON c.id = cls.coach_id
INNER JOIN "SKILL" s ON cls.skill_id = s.id
group by 專長名稱 order by coach_total desc limit 1;
...
-- 顯示須包含以下欄位： 總營收
SELECT SUM(price_paid) 總營收 FROM "CREDIT_PURCHASE" WHERE purchase_at BETWEEN '2024-12-01' AND '2024-12-31';
